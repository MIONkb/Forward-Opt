...
//
// FORWARD-MLIR is licensed under the 2-Clause BSD License except for the
...
int64_t applyMultiplierAndRShift(int64_t v, int64_t multiplier, int64_t rshift,
                                 FORWARD::RequantMode qmode, RoundingMode rmode) {
  switch (qmode) {
  case FORWARD::RequantMode::MultiplierShift:
    ...
    //   return RightShiftRound(v * multiplier, (int)rshift, rmode);
    // }
  case FORWARD::RequantMode::OnlyShift:
    return RightShiftRound(v, (int)rshift, rmode);
  case FORWARD::RequantMode::QDM:
  case FORWARD::RequantMode::TFLite:
  case FORWARD::RequantMode::TFLite_LShift:
    // if (module::isCV18xx()) {
    //   rshift = -rshift;
    // }
