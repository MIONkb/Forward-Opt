...
static inline bf16 fp32_to_bf16_denorm(fp32 src, RoundingMode round_mode) {
  ...
  if (((src.format.frac >> 16) & 0x7f) == 0x7f) {
    if ((src.format.frac & 0xffff) == 0x0) { // 0x007f0000
      ...
      if ((round_mode == FORWARD::ROUNDING_HALF_TO_EVEN) ||
          (round_mode == FORWARD::ROUNDING_HALF_AWAY_FROM_ZERO) ||
          (round_mode == FORWARD::ROUNDING_TOWARDS_ZERO)) {
        dst.bits = 0x0;
      } else if (round_mode == FORWARD::ROUNDING_DOWN) {
      ...
      if ((round_mode == FORWARD::ROUNDING_HALF_TO_EVEN) ||
          (round_mode == FORWARD::ROUNDING_HALF_AWAY_FROM_ZERO)) {
        dst.bits = 0x80;
      } else if (round_mode == FORWARD::ROUNDING_TOWARDS_ZERO) {
        dst.bits = 0x0;
      } else if (round_mode == FORWARD::ROUNDING_DOWN) {
        if (src.format.sign == 0) {
          dst.bits = 0x0;
...
static inline bf16 fp32_to_bf16_all(fp32 &src, RoundingMode round_mode) {
  ...
  if (src.format.exp > 0 && src.format.exp < 255) {
    ...
    if (round_mode == FORWARD::ROUNDING_DOWN) {
      if (src.format.sign == 0) {
        temp_r = (src.bits >> 16);
      } else {
        temp_r = ((src.bits >> 16) + (mant != 0));
      }
    } else if (round_mode == FORWARD::ROUNDING_UP) {
      if (src.format.sign == 0) {
        temp_r = ((src.bits >> 16) + (mant != 0));
      } else {
        temp_r = (src.bits >> 16);
...
static uint16_t bm_f32_to_bf16(float src) {
  fp32 tmp = {.fval = src};
  bf16 ret = fp32_to_bf16_all(tmp, FORWARD::ROUNDING_HALF_TO_EVEN);
  return ret.bits;
}
...
uint16_t f32_to_f16(float src) {
  fp32 tmp = {.fval = src};
  fp16 ret = fp32_to_fp16_all(tmp, FORWARD::ROUNDING_HALF_TO_EVEN);
  return ret.bits;
}
