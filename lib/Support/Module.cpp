...
// #include "FORWARD/Backend/Arch.h"
// #include "FORWARD/Dialect/Top/IR/TopOps.h"
#include "FORWARD/Dialect/FORWARD/IR/FORWARD.h"
#include "FORWARD/Support/TensorFile.h"
#include "FORWARD/Support/MathUtils.h"
// #include "FORWARD/Interfaces/LocalGenInterface.h"
...
#include "FORWARD/Support/ModuleEnum.cpp.inc"
namespace mlir{
namespace FORWARD {
namespace module {
struct Attr {
...
void init(ModuleOp module, std::string weightnpz){
  m = module;
    wFile = nullptr;
  // llvm::errs() << " [Info] weightnpz: "<< weightnpz <<'\n';
  m->setAttr(Attr::WEIGHT_FILE, StringAttr::get(module.getContext(), weightnpz));
  init(module);
}
...
//   chip = symbolizeChip(chip_).value_or(Chip::ALL);
  ...
  // m->setAttr(Attr::WEIGHT_FILE, StringAttr::get(ctx, weightnpz));
...
FuncOp getMainFuncOp() { return getFuncOp("forward"); }
...
NameLoc getLoc(Value v) {
  ...
  if (auto loc = v.getLoc().dyn_cast<NameLoc>()) {
    ...
    auto loc = op->getLoc();
    // llvm::errs() << "[Info] op = v.getDefiningOp():" << op 
    //              << ", loc:" << loc  
    //              << ", name_loc"<<loc.dyn_cast<NameLoc>()
    //              << ", fuse_loc" << loc.dyn_cast<FusedLoc>()
    //              <<  "\n";
...
StringRef getName(Operation *op, int index) {
  auto idx = cast<StringAttr>(op->getAttr("idx")).strref();
  if(idx != ""){
    return idx;
  }
...
//   main_func.walk([&](top::InputOp op) { inputs.push_back(op.getOutput()); });//check
