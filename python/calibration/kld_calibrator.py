...
class BaseKldCalibrator:
    ...
    def __init__(self, math_lib_path='calibration_math.so'):
        # print(math_lib_path,"is not avilible")
        
        self.calib_lib = ctypes.CDLL(math_lib_path)
        self.calib_lib.kl_diversity.restype = ctypes.c_float
        self.calib_lib.kl_diversity_hist.restype = ctypes.c_float
...
class ActivationCalibrator2(BaseKldCalibrator):
    ...
    def __init__(self, args, ds: DataSelector, py_module: pymlir.py_module):
        super().__init__()
        self.args = copy.deepcopy(args)
        ...
        #         exit(1)
        # print(args.mlir_file)
        
        self.parser = utils.mlir_parser.MlirParser(self.mlir_file)
        # print("afterparse",self.parser.module)
        self.batch_size = self.parser.get_batch_size()
        self.input_num = self.parser.get_input_num()
        self.inputs = self.parser.inputs
        self.ppa_list = []
        self.input_names = []
        self.input_2_name = {}
        for i in range(self.input_num):
            tmp = preprocess()
            # input_name = Operation.name(self.inputs[i])
            input_name="input"+"_"+str(i)
            self.input_2_name[self.inputs[i]] = input_name
            self.input_names.append(input_name)
            shape = self.parser.get_input_shape_by_idx(i)
            tmp.load_config(self.parser.get_input_op_by_idx(i), input_name, shape)
        ...
        # log_level = "DEBUG" if 'debug_log' in self.debug_cmd else "INFO"
        self.logger = setup_logger('auto_tune', log_level="INFO")
    ...
    def load_net_input(self):
        ...
        self.dq_activations[tune_idx] = {}
        self.ref_activations[tune_idx] = {}
        only_one = len(self.input_names) == 1
        for data in self.data_list:
            print("data",data)
            if self.ds.all_npz:
                print("all npz")
                x = np.load(data)
                print("x.files: ", x.files)
                if only_one:
                    assert (len(x.files) == 1)
                    n0 = self.input_names[0]
                    n1 = x.files[0]
                    if x[n1].shape[0] > 1:
                        self.dq_activations[tune_idx][n0] = [x[n1], inp_ref_dict[n0]]
                        self.ref_activations[tune_idx][n0] = [x[n1], inp_ref_dict[n0]]
    ...
    def gen_ref_tensor(self, i, op_name):
        # print("self.ref_activations", self.ref_activations)
        print("opname:", op_name)
        if op_name in self.ref_activations[i]:
            return

        # print(self.ref_activations[i])
        
        input_ops = self.parser.get_pre_op_by_op_name(op_name)
        print("input_ops:", input_ops)
        # print("input_2_name:", self.input_2_name)
        for input_op in input_ops:
            print("tem input_op is:", input_op)
            if input_op in self.input_2_name.keys():
                data = self.ref_activations[i][self.input_2_name[input_op]][0]
                self.module.set_tensor(self.input_2_name[input_op], data)
            elif input_op in self.parser.get_computing_op_name_list():
                data = self.ref_activations[i][input_op][0]
                self.module.set_tensor(self.input_2_name[input_op], data)
            else:
                print("is not computing or input")
            # print("data:", data)
            # print("after set")
        if len(input_ops) > 0:
            print("valuable")
    ...
    def activation_collect_and_calc_th(self):
        ...
        thresholds_map_zp = {}
        all_tensors = self.parser.get_computing_op_name_list()
        print("input_names",self.input_names)
        print("all_tensors",all_tensors)
        step = (99.999999 - 99.99) / len(all_tensors)
        pbar = tqdm.tqdm(all_tensors, total=len(all_tensors), position=0, leave=True)
        for i, evaled_op in enumerate(all_tensors):
            ...
            input_ops = self.parser.get_pre_op_by_op_name(evaled_op)
            if len(input_ops) == 0:
                print("what's wrong with computing op:", evaled_op)
                continue
            
            print("finish gen_ref_tensor!!")
