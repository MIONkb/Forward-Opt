...
class preprocess(object):
    ...
    def load_config(self, input_op, input_name, shape):
        self.input_name = input_name
        # shape = Operation.shape(input_op)
        self.net_input_dims = []
        shape_lenth = shape.rank
        print("shape: ", shape)
        print("shape_lenth: ", shape_lenth)
        shape_dim_sizes = [shape.get_dim_size(i) for i in range(shape.rank)]
        if shape_lenth >= 3:
            self.net_input_dims = shape_dim_sizes[-2:]
            self.batch_size = shape_dim_sizes[0]
        elif shape_lenth == 2:
            #in some model, the input tensor's dims maybe 2
            self.net_input_dims = shape_dim_sizes[0:]
            self.batch_size = shape_dim_sizes[0]
        else:
            return
        # @xcgao: do not have attr..
